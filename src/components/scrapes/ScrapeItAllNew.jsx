...
const getTemplateData = async (page) => {
  ...
  for (const child of productElements) {
    ...
    const elementClipPathObject = { tagName };
  ...
  elementsData.forEach((item) => {
    ...
    //   data.pages[0].elements.push(createFrameStructure(item));
    // }
    if (item.firstClipPath?.type === "frame"){
      data.pages[0].elements.push(createFrameStructure(item, {children: {}, path: item.firstClipPath}));
    }
    ...
    if (item.type === "text" && item.imgClipPath?.type === "frame") {
      ...
      return data.pages[0].elements.push(createFrameStructure(item, {children: children, path: item.imgClipPath}));
    } 
    else if (item.type === "image" && item.imgClipPath === null) {
      const children = {
        type: "image",
        text: item.altName || "Image",
        width: "100%",
      ...
      return data.pages[0].elements.push(createFrameStructure(item, {children: children, path: item.firstClipPath}));
    } 
